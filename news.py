...
content4 = "AMD has announced the release of two new graphics cards, the Radeon Pro W7600 and W7500, as part of their mid-range professional video card lineup. These cards are based on AMD's RDNA 3 architecture and offer advanced features such as AV1 encoding support, improved compute and ray tracing throughput, and DisplayPort 2.1 outputs. The Radeon Pro W7600 is a full-height card with 32 compute units and a boost clock of 2.43GHz, while the W7500 is a sub-75W card that can be powered entirely by a PCIe slot. Both cards are expected to hit the market later this quarter."

content5 = "Community update: Phone 2 launched with a vlog-style keynote featuring roller coasters. Nothing OS 2.0 emphasizes mindful optimization. $96 million raised for operations and future projects. CMF by Nothing, a new brand, emphasizes accessible design. Community board addressed user feedback and upcoming software features."

content6 = " 一加手机岛 23-8-4 08:31 发布于广东 来自一加Ace2Pro 性能巅峰 淘汰 8GB， 12GB 起步， 16GB 普及， 24GB 引领，还有呢？ #一加Ace2Pro#，2023年8月，敬请期待。 关注＋转评，抽送一个一加真无线耳机。 8GB 淘汰 12GB 起步 116GB 普及 24GB 引领"
...
URL:https://youtu.be/QgK0vCHZAIk
summary: {content5}
answer:
[Nothing](https://youtu.be/QgK0vCHZAIk) 推出子品牌 CMF，将于年底发布智能手表和耳机
...
answer:
    ...
    return llm_chain({"content1": content1, "content2": content2, "content3": content3, "content4": content4,
                      "content5": content5, "content6": content6, "content7": content, "url7": url})


def summary(url):
    content = "url\n"+loadWeb(url)
    prompt_template = "You are a good news writer. Summarize the following news in 100 words. Ensure you get the details and accuacy. Avoid misleadings. News: {news}"
    llm = OpenAI(temperature=0, model_name="gpt-3.5-turbo-16k")
    llm_chain = LLMChain(
        llm=llm,
        prompt=PromptTemplate.from_template(prompt_template)
...
def main():
    ...
    if st.button("生成"):
        if selector == "URL":
            ...
            st.text_area(label="早晚报草稿", value="# News\n\n☀️ 自留地早报【"+str(datetime.date.today().month)+"."+str(datetime.date.today().day)+"】\n\n" +
                         "\n\n".join(news_content)+"\n\n频道：@ NewlearnerChannel", height=1000)
